$(function() {
  ...
  function bid (c) {
    var message =  parseInt(c,10) + parseInt($('.show:last-child').find('.messageBody').text(),10);
    var now = moment();
    // Prevent markup from being injected into the message
    message = cleanInput(message);
    // if there is a non-empty message and a socket connection
  ...
  // Update the relative time stamps on the current bid every minute

  setInterval(function(){

    biddingTime.each(function(){
      var each = moment($('.bidding_time').data('time'));
      var ms = each.valueOf();

      $(this).text(each.utc(+ms).format("HH:mm:ss.SSS"));
  ...
  function addChatMessage (data, options) {
    ...
    var $messageBodyDiv = $('<span class="messageBody">')
      .text(data.message);
    //AA@MI[append bid time in miliseconds]
    var $bidTime = $('<i class="bidding_time" data-time=' + data.now + '></i>');

    var typingClass = data.typing ? 'typing' : '';
    var $messageDiv = $('<li class="message"/>')
      .data('username', data.username)
      .addClass(typingClass)
      .append($usernameDiv, $messageBodyDiv,$bidTime);
  ...
  function addMessageElement (el, options) {
    ...
    //AA@MI[convert time into miliseconds and format it later]
    var now = moment();
    var ms = now.valueOf();
    biddingTime = $(".bidding_time");
    biddingTime.last().text(now.utc(+ms).format("HH:mm:ss.SSS"));
